...
function LoginForm({setAuth}) {
    ...
    const handleSubmit = async (e) => {
        ...
        .catch((err) =>{
            console.log(err)
            setErrMsg('Please, check email and Password again!');
        })
    }
    ...
    <div className='space-y-10'>
        <form onSubmit={handleSubmit} action="" className='flex flex-col gap-5 place-items-center'>
            ...
            <fieldset className='flex items-center border py-2 px-2 rounded-md active:outline w-4/5 md:w-2/3 gap-1'>
                ...
                <input value={email} required onChange={(e)=>setEmail(e.target.value)} type="email" autoFocus placeholder="user@mail.com" className='bg-transparent placeholder:text-place outline-none h-10 w-full' />
            </fieldset>
            <fieldset className='flex items-center border py-2 px-2 rounded-md active:outline w-4/5 md:w-2/3 gap-1'>
                <LockClosedIcon className='h-8 text-place' />
                <input value={password} required onChange={(e)=>setPassword(e.target.value)} type="password" placeholder="Password" className='bg-transparent placeholder:text-place outline-none h-10 w-full' />
            </fieldset>
            ...
            <button onClick={handleSubmit} className='' class="btn btn-primary">Sign In</button>
        </form>
