...
function DashMenu() {
  ...
  const [safetyAndHealth, setSafetyandHealth] = useState(false);
  const safetyandhealthSubMenu = (show) => {
    setSafetyandHealth((show) => !show);
  };
        ...
        <div className="space-y-16 pl-12">
          <div className="relative grid gap-5 w-4/5">
            ...
            >
              ...
              <ChevronDownIcon
                onClick={operationsSubMenu}
                className="h-6 stroke-1 hover:scale-125 transition duration-500"
              />
            </div>
            {/*safetyAndHealth*/}
            <div
              onClick={safetyandhealthSubMenu}
              className="items-center relative flex text-white gap-7 cursor-pointer"
            >
              <DashMenuItem
