...
const AbsenceForm = () => {
  PageTitle("FIRS Delivery Officer Absence Form");
      ...
      <form action="" className="grid text-primary gap-5 w-full">
        <fieldset className="grid md:grid-cols-2 gap-3 items-end">
          ...
          <label htmlFor="">
            ...
            <div className="border border-primary h-12 rounded-lg grid items-center">
              {trucks
                ...
                .map((item) => (
                  <span className="px-4">{item.deliveryOfficer}</span>
                ))}
            </div>
          </label>
          <label htmlFor="">
            ...
            <div className="border border-primary h-12 rounded-lg grid items-center">
              {trucks
                .filter((t) => t.truckId === data.truckId)
                .map((item) => (
                  <span className="px-4">{item.journeyOfficer}</span>
                ))}
            </div>
          </label>
          <label htmlFor="">
            ...
            <div className="border border-primary h-12 rounded-lg grid items-center">
              {trucks
                .filter((t) => t.truckId === data.truckId)
                .map((item) => (
                  <span className="px-4">{item.logisticsCoordinator}</span>
                ))}
            </div>
          </label>
          <label htmlFor="">
          ...
          <label htmlFor="">
            ...
            <select
              value={data.alternativeDeliveryOfficerId}
              onChange={(e) =>
                setData({
                  ...data,
                  alternativeDeliveryOfficerId: e.target.value,
                })
              }
